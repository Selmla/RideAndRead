...
const LS_KEY = "rideandread_invite";

export default function App() {
  const hasPending = !window.location.hash && !!localStorage.getItem(LS_KEY);
  const [screen, setScreen] = useState(window.location.hash || hasPending ? "loading" : "create");
  ...
  useEffect(() => {
    ...
    if (hash) {
      // Recipient (or sender returning via shared link)
      if (hash.startsWith("inv_")) {
        ...
        getDoc(doc(db, "invitations", id)).then((snap) => {
          ...
          if (d.status === "cancelled") { setScreen("invalid"); return; }
          if (d.status === "accepted") {
            setRequestData(d);
            setChosenDate(d.chosenDate);
            setChosenActivity(ACTIVITIES.find(a => a.id === d.chosenActivityId));
            setScreen("confirmed"); return;
          }
          if (d.status === "declined") {
            setRequestData(d); setScreen("declined"); return;
          }
          setRequestData(d);
          setInviteId(id);
          setIsRecipient(true);
        ...
        if (data) { setRequestData(data); setIsRecipient(true); setScreen("recipient"); }
        else setScreen("invalid");
      }
    } else {
      // No hash — check if sender has a pending invite in localStorage
      const pendingId = localStorage.getItem(LS_KEY);
      if (!pendingId) { setScreen("create"); return; }

      getDoc(doc(db, "invitations", pendingId)).then((snap) => {
        if (!snap.exists()) { localStorage.removeItem(LS_KEY); setScreen("create"); return; }
        const d = snap.data();
        if (d.status === "pending") {
          setRequestData(d);
          setInviteId(pendingId);
          setShareUrl(`${window.location.origin}${window.location.pathname}#inv_${pendingId}`);
          setScreen("link");
  ...
  // Sender: real-time listener for recipient's response
  useEffect(() => {
  ...
  const handleYes = async (date, activity) => {
    ...
    if (inviteId) {
      await updateDoc(doc(db, "invitations", inviteId), {
        status: "accepted", chosenDate: date, chosenActivityId: activity.id
      });
    }
    setScreen("confirmed");
  };
  ...
  const headerLabel = {
    ...
    confirmed: "It's a date", no: "", declined: "", invalid: "", error: "", loading: ""
  }[screen] ?? "";

  return (
    <div style={{ minHeight: "100vh", color: "#E8E0D0", fontFamily: "'Crimson Text', Georgia, serif", background: "#0D0D0D" }}>
      <style>{`
      ...
      <div style={{ position: "fixed", inset: 0, zIndex: 0, backgroundImage: "url('/darkromancebg.png')", backgroundSize: "cover", backgroundPosition: "center top" }} />
      <div style={{ position: "fixed", inset: 0, zIndex: 0, background: "linear-gradient(rgba(8,4,4,0.72) 0%, rgba(8,4,4,0.65) 100%)" }} />
      <GrainOverlay />

      <div style={{ maxWidth: "420px", margin: "0 auto", padding: "0 20px 60px", position: "relative", zIndex: 1 }}>
        <div style={{ padding: "20px 0 16px", display: "flex", justifyContent: "space-between", alignItems: "center", borderBottom: "1px solid #252525", marginBottom: "8px" }}>
          <Logo />
        ...
        {!isRecipient && screen === "link" && <StepDots current={1} total={2} />}

        {screen === "loading" && (
          <div style={{ animation: "fadeUp 0.5s ease both", textAlign: "center", padding: "80px 0" }}>
            <span style={{ display: "inline-block", width: "10px", height: "10px", borderRadius: "50%", background: "#6B1E1E", animation: "pulse 1.8s ease-in-out infinite", marginBottom: "24px" }} />
        ...
        {screen === "link" && <LinkScreen data={requestData} url={shareUrl} onCancel={handleCancel} />}
        ...
        {screen === "declined" && (
          <div style={{ animation: "fadeUp 0.5s ease both", textAlign: "center", padding: "60px 0" }}>
            ...
            <p style={{ fontFamily: "'Playfair Display', Georgia, serif", fontSize: "26px", color: "#E8E0D0", fontStyle: "italic", margin: "0 0 16px", lineHeight: 1.4 }}>Not this chapter.</p>
            ...
            <button onClick={() => { localStorage.removeItem(LS_KEY); setScreen("create"); }} style={{
              background: "none", border: "1px solid #383838", borderRadius: "8px", color: "#707070",
              fontFamily: "Georgia, serif", fontSize: "12px", letterSpacing: "0.1em",
              textTransform: "uppercase", cursor: "pointer", padding: "14px 24px"
            }}>Send a new invitation</button>
          </div>
        )}
        ...
        {screen === "error" && (
          <div style={{ animation: "fadeUp 0.5s ease both", textAlign: "center", padding: "60px 0" }}>
            <div style={{ fontSize: "32px", marginBottom: "24px", opacity: 0.4 }}>📡</div>
            <p style={{ fontFamily: "'Playfair Display', Georgia, serif", fontSize: "26px", color: "#E8E0D0", fontStyle: "italic", margin: "0 0 16px", lineHeight: 1.4 }}>Something went wrong.</p>
            <p style={{ fontFamily: "'Crimson Text', Georgia, serif", fontSize: "16px", color: "#707070", lineHeight: 1.7, margin: "0 0 40px" }}>Check your connection and try again.</p>
        ...
        {screen === "invalid" && (
          <div style={{ animation: "fadeUp 0.5s ease both", textAlign: "center", padding: "60px 0" }}>
            ...
            <p style={{ fontFamily: "'Playfair Display', Georgia, serif", fontSize: "26px", color: "#E8E0D0", fontStyle: "italic", margin: "0 0 16px", lineHeight: 1.4 }}>This invitation is no longer valid.</p>
            <p style={{ fontFamily: "'Crimson Text', Georgia, serif", fontSize: "16px", color: "#707070", lineHeight: 1.7, margin: "0 0 40px" }}>
            ...
            <button onClick={() => { window.location.hash = ""; setScreen("create"); }} style={{
              ...
              fontFamily: "Georgia, serif", fontSize: "12px", letterSpacing: "0.1em",
              textTransform: "uppercase", cursor: "pointer", padding: "14px 24px"
            }}>Create a new invitation</button>
          </div>
        )}
